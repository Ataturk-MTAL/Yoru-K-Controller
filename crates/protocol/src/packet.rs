...
pub fn speed_packet(left: i8, right: i8, gear: u8, rev_left: bool, rev_right: bool) -> Vec<u8> {
    ...
    let r = if rev_right { right.saturating_neg() } else { right }.clamp(-100, 100);
...
pub fn light_packet(on: bool) -> Vec<u8> {
    let pkt = vec![START_BYTE, Command::SetLight as u8, 0x01, if on { 0x01 } else { 0x00 }];
    let ck = fletcher16(&pkt[1..]);
    let mut result = pkt;
    result.extend_from_slice(&ck);
    result
...
pub fn brake_packet(on: bool) -> Vec<u8> {
    let pkt = vec![START_BYTE, Command::SetBrake as u8, 0x01, if on { 0x01 } else { 0x00 }];
    let ck = fletcher16(&pkt[1..]);
    let mut result = pkt;
    result.extend_from_slice(&ck);
    result
...
pub fn parse_response(data: &[u8]) -> Option<RobotResponse> {
    ...
    let payload = if data.len() >= 3 + len { &data[3..3 + len] } else { return None };
