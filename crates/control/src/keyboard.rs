...
/// İleri/geri tam hız (100), dönüş yarım hız (50) olarak uygulanır
pub fn calculate(state: &KeyboardState) -> MotorSpeeds {
    ...
    let turn_l   = if state.left     { 50i32  } else { 0 };
    let turn_r   = if state.right    { 50i32  } else { 0 };
...
mod tests {
    ...
    fn test_turn_right() {
        ...
        assert!(s.left > s.right, "Sağa dönüşte sol > sağ olmalı");
    }
