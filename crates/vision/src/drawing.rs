...
/// Sabit yeşil renk — tüm detection kutuları ve etiketler için
const GREEN: Rgba<u8> = Rgba([0, 230, 64, 255]);
...
pub fn draw_detections(img: &mut RgbaImage, detections: &[Detection]) {
    ...
    for det in detections {
        ...
        // Bounding box: 3px kalınlık — yeşil
        for t in 0..3i32 {
            let w = width.saturating_sub(t as u32 * 2);
            let h = height.saturating_sub(t as u32 * 2);
            if w > 0 && h > 0 {
                draw_hollow_rect_mut(img, Rect::at(x + t, y + t).of_size(w, h), GREEN);
            }
        }

        // Etiket — vurgulu (büyük font, kalın arka plan)
        let label = format!("{} {:.0}%", det.class_name, det.confidence * 100.0);
        let label_y = y.saturating_sub(24).max(0);

        if let Some(ref f) = font {
            draw_text_glyph(img, x, label_y, &label, GREEN, f, 16.0);
        } else {
            draw_text_minimal(img, x, label_y, &label, GREEN);
        }
    }
}
...
fn draw_text_glyph(
    ...
    // Koyu arka plan — yüksek opak
    let bx = x.max(0) as u32;
    let by = y.max(0) as u32;
    let bw = (text_w as u32 + 10).min(img.width().saturating_sub(bx));
    let bh = (text_h as u32 + 4).min(img.height().saturating_sub(by));
    for dy in 0..bh {
        for dx in 0..bw {
            blend_pixel(img, bx + dx, by + dy, Rgba([0, 0, 0, 255]), 0.8);
        }
    }

    // Glyph rasterizasyonu
    let mut pen_x      = x as f32 + 5.0;
    let baseline_y     = y as f32 + ascent + 3.0;
